...
namespace zt::callback
{
    ...
    template<>
    class BeforeEventTile<ttype::InvalidData>{
        friend class Registry;
        public:
        using ID=unsigned long long;
        using func=void(*)(zt::event::tile::params<ttype::InvalidData>&);
        private:
        IDMaker<ID> maker;
        std::vector<std::pair<ID,func>> queueOfEvent;
        //std::unordered_map<ID,void(*)(zt::event::entity::params<T>&,bool&)> EventbyIterator;
        public:
        BeforeEventTile(){}
        ID EventSubscribe(void(*eventRes)(zt::event::tile::params<ttype::InvalidData>&)){
            ID id;
            if(!maker.getID(id))throw std::logic_error("what the ... how many event that you made?dude!");
            queueOfEvent.emplace_back(id,eventRes);
            return id;
        ...
        void emit(zt::event::tile::params<ttype::InvalidData>& params){
            for (size_t i = 0; i < queueOfEvent.size(); ){
                auto id = queueOfEvent[i].first;
                auto handler = queueOfEvent[i].second;
                handler(params);
    ...
    class TileEventListener{
        ...
        TileEventListener(){};
        std::tuple<
            AfterEventTile<ttype::EntityStepOn>,AfterEventTile<ttype::PlayerInteractWithTile>,
        ...
        > beforeEvent;
        std::tuple<
            ComponentRegisterofTile<ttype::EntityStepOn>,ComponentRegisterofTile<ttype::PlayerInteractWithTile>,
        ...
        > component;
        public:
        template<ttype T> AfterEventTile<T>& getAfterEvent(){return std::get<AfterEventTile<T>>(afterEvent);}
        template<ttype T> BeforeEventTile<T>& getBeforeEvent(){return std::get<BeforeEventTile<T>>(beforeEvent);}
        template<ttype T> ComponentRegisterofTile<T>& getComponent(){return std::get<ComponentRegisterofTile<T>>(this->component);}
    };
